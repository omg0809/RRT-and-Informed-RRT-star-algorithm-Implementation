...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # return point
            ...
            # c_min = np.sqrt((self.start.row - self.goal.row) ** 2 + (self.start.col - self.goal.col) ** 2)
            ...
            # Compute rotation matrix from elipse to world frame - C
            angle = np.arctan2(self.goal.col - self.start.col, self.goal.row - self.start.row)
            C = np.array([[np.cos(angle), -np.sin(angle)],
                        [np.sin(angle), np.cos(angle)]])
            # cos_theta = (self.goal.col - self.start.col) / c_min
            # sin_theta = (self.goal.row - self.start.row) / c_min
            # C = np.array([[cos_theta, -sin_theta], [sin_theta, cos_theta]])

            # Compute diagonal matrix - L
            L = np.diag([c_best, c_min / 2])

            # Cast a sample from a unit ball - x_ball
            x_ball = np.random.normal(size=(2, 1))

            # # Map ball sample to the ellipsoid - x_rand
            x_rand = x_center + C.dot(L).dot(x_ball).flatten()
            point = [int(round(x_rand[0])), int(round(x_rand[1]))]
            x_ball = np.random.normal(size=2)
            x_ball = x_ball / np.linalg.norm(x_ball)

            # Map ball sample to the ellipsoid - x_rand
            # x_rand = np.matmul(C, np.matmul(L, x_ball)) + x_center
            # while self.check_collision(x_rand,x_ball):
            #     x_ball = np.random.normal(size=2)
            #     x_ball = x_ball / np.linalg.norm(x_ball)
            #     x_rand = np.matmul(C, np.matmul(L, x_ball)) + x_center
            
            # point = Node(int(x_rand[0]), int(x_rand[1]))
    ...
    def sample(self, goal_bias=0.5, c_best=0):
